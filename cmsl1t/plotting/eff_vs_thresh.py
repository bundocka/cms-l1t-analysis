...
class EffVsThreshPlot(BasePlotter):
    drawstyle = 'PC'
    drawstyle_data = 'PC'

    def __init__(self, online_name, offline_name):
        name = ["effvsThresh", online_name, offline_name]
        super(EffVsThreshPlot, self).__init__("__".join(name))
    ...
    def create_histograms(
        ...
        self.thresholds = bn.LessThan(thresholds, "threshold")
        self.legend_title = legend_title

        name = ["effVsThresh", self.online_name, self.offline_name]
        ...
        def make_effVsThresh(labels):
            this_name = "effVsThresh" + name.format(**labels)
            ...
            eff.drawstyle = EffVsThreshPlot.drawstyle
            return eff
    ...
    def fill(self, pileup, offline, online):
        ...
        for (pu_bin, threshold_bin), efficiency in efficiencies.items():
            ...
            if isinstance(threshold, str):
                continue
            elif offline < threshold:
                continue
            nbins = efficiency.GetTotalHistogram().GetNbinsX()
            for xbin in range(1, nbins + 1):
                passed = True
                xlow = efficiency.GetTotalHistogram().GetBinLowEdge(xbin)
                if xlow >= online:
                    passed = False
                efficiency.fill(passed, xlow)
    ...
    def draw(self, with_fits=False):
        ...
        labels = []
        fits = []
        label_template = '{offline_title} > {threshold} GeV'
        for threshold in all_pileup_effs.iter_all():
            if not isinstance(threshold, int):
                continue
            hist = all_pileup_effs.get_bin_contents(threshold)
            hist.drawstyle = EffVsThreshPlot.drawstyle_data
            hists.append(hist)

            label = label_template.format(
                offline_title=self.offline_title,
        ...
        self.__make_overlay(
            "all", "all", hists, fits, labels, self.offline_title,
        )
    ...
    def overlay(self, other_plotters=None, with_fits=False):
        ...
        for threshold in all_pileup_effs.iter_all():
            ...
            label = label_template.format(
                offline_title=self.offline_title + " " + titles[0],
                threshold=self.thresholds.bins[threshold],
            )
            labels.append(label)
            if with_fits:
        ...
        for other_plotter in other_plotters:
            ...
            for threshold in pileup_effs.iter_all():
                ...
                label = label_template.format(
                    offline_title=self.offline_title + " " + titles[other_plotters.index(other_plotter) + 1],
        ...
        self.__make_overlay(
            "all", suffix, hists, fits, labels, self.offline_title,
        )
    ...
    def __make_overlay(self, pileup, threshold, hists, fits, labels, header):
        with preserve_current_style():
            ...
            if 'Jet' in self.online_title:
                xtitle = "L1 Jet #it{p}_{T} (GeV)"
            if 'HT' in self.online_title:
                xtitle = "L1 #it{H}_{T} (GeV)"
            if 'MET' in self.online_title:
                xtitle = "L1 #it{E}_{T}^{miss} (GeV)"
